...
describe('UserModel', () => {
  ...
  describe('getByEmail', () => {
    ...
    it('should throw an error if query fails', async () => {
      ...
      await expect(UserModel.getByEmail('john@example.com')).rejects.toThrow('Something went wrong');
    });
  });
  ...
  describe('getById', () => {
    ...
    it('should throw an error if query fails', async () => {
      ...
      await expect(UserModel.getById(1)).rejects.toThrow('Something went wrong');
    });
  });
  ...
  describe('createUser', () => {
    ...
    it('should throw an error if query fails', async () => {
      const error = new Error('Something went wrong');
      const newUser = { first_name: 'John', last_name: 'Doe', email: 'john@example.com', password: 'hashedpassword' };
      pool.query.mockRejectedValue(error);

      await expect(UserModel.createUser(newUser)).rejects.toThrow('Something went wrong');
    });
  });
  ...
  describe('update', () => {
    it('should update an existing user', async () => {
      ...
      const result = await UserModel.update(1, { first_name: 'John', last_name: 'Doe', email: 'john_updated@example.com' });
    ...
    it('should throw an error if query fails', async () => {
      ...
      await expect(UserModel.update(1, { first_name: 'John', last_name: 'Doe', email: 'john_updated@example.com' })).rejects.toThrow('Something went wrong');
    });
  });
